...
def find_flower_image_file(filename: str, auto_remote_fetch: bool = True) -> Optional[str]:
    ...
    6. Nếu chưa có trên đĩa và auto_remote_fetch=True: Tự động nạp từ GitHub CDN và lưu vào cache cục bộ.
    """
    ...
    candidates = [
        ...
        os.path.join(ROOT_DIR, "images", "products", clean_name),
                ...
                with urllib.request.urlopen(req, timeout=4) as resp:
                    if resp.status == 200:
                        ...
                        with open(target_save, "wb") as f:
                            f.write(content)
                        logger.info(f"Đã fetch và lưu ảnh từ GitHub CDN vào cache: {clean_name}")
                        return target_save
            except Exception:
                continue
