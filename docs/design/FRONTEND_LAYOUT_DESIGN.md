...
| :---: | :--- | :---: | :--- |
| **01** | [`js/portal_admin_state.js`](file:///d:/wmshare/telua_flower/js/portal_admin_state.js) | 68 | Khai báo State chia sẻ chung (`allAdminCategories`, `allAdminBranches`, `allAdminStaff`, `allAdminPriceLevels`), quản lý cấu hình phân tầng giá động `PRICE_LEVEL_CONFIG` (xóa bỏ hoàn toàn hardcode, nạp động qua `setAdminPriceLevels`), các tiện ích `lockScreen`, `unlockScreen`, `notifyUser`. |
...
| **05** | [`js/portal_admin_products.js`](file:///d:/wmshare/telua_flower/js/portal_admin_products.js) | 1205 | CMS quản lý mẫu hoa, nạp động phân tầng giá cho dropdown (`populatePriceLevelSelect`), kiểm tra hàng rào giá an toàn theo Price Levels, tải ảnh Base64 & upload ảnh thật, bộ sưu tập gallery, render động định ngạch tồn kho (`renderProductModalStockFields`). |
...
| **08** | [`js/portal_admin_sysconfig.js`](file:///d:/wmshare/telua_flower/js/portal_admin_sysconfig.js) | 1045 | Cấu hình thông tin công ty (`infoCompany.json`), công tắc bật/tắt các phương thức thanh toán, chính sách hiển thị phụ kiện, trình quản lý ảnh Slider Banner trang chủ, và quản lý toàn diện phân tầng mức giá (`price_levels.json`: CRUD, modal thêm/sửa, kiểm tra số liệu thời gian thực). |
